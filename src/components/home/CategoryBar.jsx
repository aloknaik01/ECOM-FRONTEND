...
const CategoryBar = () => {
  ...
  return (
    <div className="bg-white dark:bg-gray-900 shadow-sm border-b border-gray-100 dark:border-gray-800 transition-colors duration-200 overflow-x-auto no-scrollbar py-2 sm:py-3">
      <div className="max-w-[1400px] mx-auto flex items-center justify-between px-2 sm:px-6 gap-3 sm:gap-5 min-w-max">
        {categories.map((item, index) => {
          ...
          return (
            <button
              ...
              className="flex flex-col items-center gap-1.5 sm:gap-2 group cursor-pointer transition-all min-w-[60px] sm:min-w-[72px]"
            >
              {/* Icon Container */}
              <div
                className={`w-[44px] h-[44px] sm:w-[52px] sm:h-[52px] rounded-full flex items-center justify-center transition-all duration-300 ease-out group-hover:scale-110 group-active:scale-95 ${isFeatured
              ...
              >
                <item.icon strokeWidth={1.5} className="w-[18px] h-[18px] sm:w-[22px] sm:h-[22px]" />
              </div>

              {/* Label */}
              <span
                className={`text-[10px] sm:text-[12px] font-semibold tracking-wide transition-colors duration-200 whitespace-nowrap ${isFeatured
